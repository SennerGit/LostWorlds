...
public class MagicGorganProjectileEntity extends Projectile {
    private static final EntityDataAccessor<Boolean> HIT =
            SynchedEntityData.defineId(MagicGorganProjectileEntity.class, EntityDataSerializers.BOOLEAN);
    private int counter = 0;

    public MagicGorganProjectileEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public MagicGorganProjectileEntity(Level pLevel, Player player) {
        super(ModEntities.MAGIC_GORGAN_PROJECTILE.get(), pLevel);
        setOwner(player);
...
//            this.onHit(hitresult);
        ...
        for (int i = 1; i < 5; ++i) {
            this.level().addParticle(ModParticles.MAGICAL_DOWSING_ROD_PARTICLES.get(), d0 - (d5 * 2), d1 - (d6 * 2), d2 - (d7 * 2),
                    -d5, -d6 - 0.1D, -d7);
        }
    ...
    protected void onHitEntity(EntityHitResult hitResult) {
        ...
        this.level().playSound(null, this.getX(), this.getY(), this.getZ(), ModSounds.MAGICAL_DOWSING_ROD_FOUND_ORE.get(), SoundSource.NEUTRAL,
                2F, 1F);
    ...
    protected void onHit(HitResult hitResult) {
        ...
        for (int x = 0; x < 18; ++x) {
            for (int y = 0; y < 18; ++y) {
                this.level().addParticle(ModParticles.MAGICAL_DOWSING_ROD_PARTICLES.get(), this.getX(), this.getY(), this.getZ(),
