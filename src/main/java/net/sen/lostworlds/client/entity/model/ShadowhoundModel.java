...
//import net.sen.lostworlds.client.entity.animations.ModShadowhoundAnimationDefinitions;
import net.sen.lostworlds.entity.mob.ShadowhoundEntity;

public class ShadowhoundModel<T extends ShadowhoundEntity> extends HierarchicalModel<T> {
	// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(LostWorldsApi.modLoc("shadowhound_model"), "main");
	private final ModelPart shadowhound;
	private final ModelPart head;

	public ShadowhoundModel(ModelPart root) {
		this.shadowhound = root.getChild("shadowhound");
		this.head = shadowhound.getChild("body").getChild("torso").getChild("head");
	}
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition shadowhound = partdefinition.addOrReplaceChild("shadowhound", CubeListBuilder.create(), PartPose.offset(0.0F, 24.0F, 2.5F));

		PartDefinition body = shadowhound.addOrReplaceChild("body", CubeListBuilder.create(), PartPose.offset(0.0F, 0.0F, 0.0F));
	...
	@Override
	public void setupAnim(ShadowhoundEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.root().getAllParts().forEach(ModelPart::resetPose);

		this.applyHeadRotation(entity, netHeadYaw, headPitch, ageInTicks);

//		this.animateWalk(ModShadowhoundAnimationDefinitions.RHINO_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
//		this.animate(entity.idleAnimationState, ModShadowhoundAnimationDefinitions.RHINO_IDLE, ageInTicks, 1f);
//		this.animate(entity.attackAnimationState, ModShadowhoundAnimationDefinitions.RHINO_ATTACK, ageInTicks, 1f);
//		this.animate(entity.sitAnimationState, ModShadowhoundAnimationDefinitions.RHINO_SIT, ageInTicks, 1f);
	}

	private void applyHeadRotation(ShadowhoundEntity entity, float netHeadYaw, float headPitch, float ageInTicks) {
	...
	public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
		shadowhound.render(poseStack, vertexConsumer, packedLight, packedOverlay, red, green, blue, alpha);
	}
