...
//import net.sen.lostworlds.client.entity.animations.ModSpectralhoundAnimationDefinitions;
import net.sen.lostworlds.entity.mob.SpectralhoundEntity;

public class SpectralhoundModel<T extends SpectralhoundEntity> extends HierarchicalModel<T> {
	// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(LostWorldsApi.modLoc("spectralhound_model"), "main");
	private final ModelPart spectralhound;
	private final ModelPart head;

	public SpectralhoundModel(ModelPart root) {
		this.spectralhound = root.getChild("spectralhound");
		this.head = spectralhound.getChild("body").getChild("torso").getChild("head");
	}
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition spectralhound = partdefinition.addOrReplaceChild("spectralhound", CubeListBuilder.create(), PartPose.offset(0.0F, 24.0F, 2.5F));

		PartDefinition body = spectralhound.addOrReplaceChild("body", CubeListBuilder.create(), PartPose.offset(0.0F, 0.0F, 0.0F));
	...
	@Override
	public void setupAnim(SpectralhoundEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.root().getAllParts().forEach(ModelPart::resetPose);

		this.applyHeadRotation(entity, netHeadYaw, headPitch, ageInTicks);

//		this.animateWalk(ModSpectralhoundAnimationDefinitions.SPECTRALHOUND_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
//		this.animate(entity.idleAnimationState, ModSpectralhoundAnimationDefinitions.SPECTRALHOUND_IDLE, ageInTicks, 1f);
//		this.animate(entity.attackAnimationState, ModSpectralhoundAnimationDefinitions.SPECTRALHOUND_ATTACK, ageInTicks, 1f);
//		this.animate(entity.sitAnimationState, ModSpectralhoundAnimationDefinitions.SPECTRALHOUND_SIT, ageInTicks, 1f);
	}

	private void applyHeadRotation(SpectralhoundEntity entity, float netHeadYaw, float headPitch, float ageInTicks) {
	...
	public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
		spectralhound.render(poseStack, vertexConsumer, packedLight, packedOverlay, red, green, blue, alpha);
	}
