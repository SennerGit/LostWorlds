...
//import net.sen.lostworlds.client.entity.animations.ModCaveMoleAnimationDefinitions;
import net.sen.lostworlds.entity.mob.CaveMoleEntity;

public class CaveMoleModel<T extends CaveMoleEntity> extends HierarchicalModel<T> {
	// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(LostWorldsApi.modLoc("cave_mole_model"), "main");
	private final ModelPart cave_mole;
	private final ModelPart head;

	public CaveMoleModel(ModelPart root) {
		this.cave_mole = root.getChild("cave_mole");
		this.head = cave_mole.getChild("body").getChild("torso").getChild("head");
	}
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition cave_mole = partdefinition.addOrReplaceChild("cave_mole", CubeListBuilder.create(), PartPose.offset(0.0F, 24.0F, 2.5F));

		PartDefinition body = cave_mole.addOrReplaceChild("body", CubeListBuilder.create(), PartPose.offset(0.0F, 0.0F, 0.0F));
	...
	@Override
	public void setupAnim(CaveMoleEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.root().getAllParts().forEach(ModelPart::resetPose);

		this.applyHeadRotation(entity, netHeadYaw, headPitch, ageInTicks);

//		this.animateWalk(ModCaveMoleAnimationDefinitions.RHINO_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
//		this.animate(entity.idleAnimationState, ModCaveMoleAnimationDefinitions.RHINO_IDLE, ageInTicks, 1f);
//		this.animate(entity.attackAnimationState, ModCaveMoleAnimationDefinitions.RHINO_ATTACK, ageInTicks, 1f);
//		this.animate(entity.sitAnimationState, ModCaveMoleAnimationDefinitions.RHINO_SIT, ageInTicks, 1f);
	}

	private void applyHeadRotation(CaveMoleEntity entity, float netHeadYaw, float headPitch, float ageInTicks) {
	...
	public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
		cave_mole.render(poseStack, vertexConsumer, packedLight, packedOverlay, red, green, blue, alpha);
	}
