...
public class EbonyKingswoodFoliagePlacer extends FoliagePlacer {
    public static final Codec<EbonyKingswoodFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, EbonyKingswoodFoliagePlacer::new));

    private final IntProvider trunk_height;

    public EbonyKingswoodFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.EBONY_KINGSWOOD_FOLIAGE_PLACER.get();
    }
