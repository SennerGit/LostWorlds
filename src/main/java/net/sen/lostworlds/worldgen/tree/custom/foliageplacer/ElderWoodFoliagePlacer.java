...
public class ElderWoodFoliagePlacer extends FoliagePlacer {
    public static final Codec<ElderWoodFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, ElderWoodFoliagePlacer::new));

    private final IntProvider trunk_height;

    public ElderWoodFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.ELDER_WOOD_FOLIAGE_PLACER.get();
    }
