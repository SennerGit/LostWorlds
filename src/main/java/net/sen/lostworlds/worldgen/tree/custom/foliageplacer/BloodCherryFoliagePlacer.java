...
public class BloodCherryFoliagePlacer extends FoliagePlacer {
    public static final Codec<BloodCherryFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, BloodCherryFoliagePlacer::new));

    private final IntProvider trunk_height;

    public BloodCherryFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.BLOOD_CHERRY_FOLIAGE_PLACER.get();
    }
