...
public class DeadwoodFoliagePlacer extends FoliagePlacer {
    public static final Codec<DeadwoodFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, DeadwoodFoliagePlacer::new));

    private final IntProvider trunk_height;

    public DeadwoodFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.DEADWOOD_FOLIAGE_PLACER.get();
    }
