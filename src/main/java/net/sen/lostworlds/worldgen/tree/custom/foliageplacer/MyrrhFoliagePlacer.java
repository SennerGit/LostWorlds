...
public class MyrrhFoliagePlacer extends FoliagePlacer {
    public static final Codec<MyrrhFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, MyrrhFoliagePlacer::new));

    private final IntProvider trunk_height;

    public MyrrhFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.MYRRH_FOLIAGE_PLACER.get();
    }
