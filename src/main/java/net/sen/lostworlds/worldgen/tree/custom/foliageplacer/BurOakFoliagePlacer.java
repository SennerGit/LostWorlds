...
public class BurOakFoliagePlacer extends FoliagePlacer {
    public static final Codec<BurOakFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, BurOakFoliagePlacer::new));

    private final IntProvider trunk_height;

    public BurOakFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.BUR_OAK_FOLIAGE_PLACER.get();
    }
