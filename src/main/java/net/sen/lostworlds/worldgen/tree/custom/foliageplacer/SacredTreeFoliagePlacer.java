...
public class SacredTreeFoliagePlacer extends FoliagePlacer {
    public static final Codec<SacredTreeFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, SacredTreeFoliagePlacer::new));

    private final IntProvider trunk_height;

    public SacredTreeFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.SACRED_TREE_FOLIAGE_PLACER.get();
    }
