...
public class WeaverFoliagePlacer extends FoliagePlacer {
    public static final Codec<WeaverFoliagePlacer> CODEC = RecordCodecBuilder.create((instance) -> foliagePlacerParts(instance)
                    .and(IntProvider.codec(0, 16).fieldOf("trunk_height").forGetter((placer) -> placer.trunk_height))
                    .apply(instance, WeaverFoliagePlacer::new));

    private final IntProvider trunk_height;

    public WeaverFoliagePlacer(IntProvider pRadius, IntProvider pOffset, IntProvider height) {
    ...
    protected FoliagePlacerType<?> type() {
        return ModFoliagePlacerTypes.WEAVER_FOLIAGE_PLACER.get();
    }
